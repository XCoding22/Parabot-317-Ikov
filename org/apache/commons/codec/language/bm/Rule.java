 ...
 * A phoneme rule.
 * <p>
 * Rules have a pattern, left context, right context, output phoneme, set of
 * languages for which they apply and a logical flag indicating if all languages
 * must be in play. A rule matches if:
 * <ul>
 * <li>the pattern matches at the current position</li>
 * <li>the string up until the beginning of the pattern matches the left
 * context</li>
 ...
 * </ul>
 * <p>
 * Rules are typically generated by parsing rules resources. In normal use,
 * there will be no need for the user to explicitly construct their own.
 * <p>
 * Rules are immutable and thread-safe.
 * <p>
 * <b>Rules resources</b>
 * <p>
 * Rules are typically loaded from resource files. These are UTF-8 encoded text
 ...
 * <ul>
 * <li><b>Rules:</b> whitespace separated, double-quoted strings. There should
 * be 4 columns to each row, and these will be interpreted as:
 * <ol>
 * <li>pattern</li>
 * <li>left context</li>
 * <li>right context</li>
...
public class Rule {
	...
	public static final class Phoneme implements PhonemeExpr {
		...
		/**
		 * Returns a new Phoneme with the same text but a union of its current language
		 * set and the given one.
		 *
		 * @param lang
		 *            the language set to merge
		 * @return a new Phoneme
		 */
	...
	/**
	 * A minimal wrapper around the functionality of Pattern that we use, to allow
	...
	private static String createResourceName(final NameType nameType, final RuleType rt, final String lang) {
		return String.format("org/apache/commons/codec/language/bm/%s_%s_%s.txt", nameType.getName(), rt.getName(),
				lang);
	}
	 ...
	 * @param langs
	 *            the set of languages to consider
	 * @return a map containing all Rules that apply, grouped by the first character
	 *         of the rule pattern
	 * @since 1.9
	 */
				...
				if (line.startsWith(ResourceConstants.EXT_CMT_START)) {
					...
					if (line.startsWith(HASH_INCLUDE)) {
						...
						try {
							...
							throw new IllegalStateException("Problem parsing line '" + currentLine + "' in " + location,
									e);
						}
					}
				}
	...
	/**
	 * Attempts to compile the regex into direct string ops, falling back to Pattern
	 * and Matcher in the worst case.
	 *
	 * @param regex
	...
	/**
	 * Gets the left context. This is a regular expression that must match to the
	 * left of the pattern.
	 *
	...
	/**
	 * Gets the phoneme. If the rule matches, this is the phoneme associated with
	 * the pattern match.
	 *
	 * @return the phoneme
	 */
	...
	/**
	 * Gets the right context. This is a regular expression that must match to the
	...
	/**
	 * Decides if the pattern and context match the input starting at a position. It
	 * is a match if the <code>lContext</code> matches <code>input</code> up to
	 * <code>i</code>, <code>pattern</code> matches at i and <code>rContext</code>
	 * matches from the end of the match of <code>pattern</code> to the end of
	 * <code>input</code>.
	 *
	 * @param input
	 *            the input String
