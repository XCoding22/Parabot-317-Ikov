...
public class Crypt {
	 ...
	 * </p>
	 * <p>
	 * A salt is generated for you using {@link ThreadLocalRandom}; for more secure
	 * salts consider using {@link SecureRandom} to generate your own salts and
	 * calling {@link #crypt(byte[], String)}.
	 * </p>
	 *
	 * @param keyBytes
	 *            plaintext password
	 ...
	 * Encrypts a password in a crypt(3) compatible way.
	 * <p>
	 * If no salt is provided, a random salt and the default algorithm (currently
	 * SHA-512) will be used. See {@link #crypt(String, String)} for details.
	 *
	 * @param keyBytes
	 *            plaintext password
	 * @param salt
	 *            real salt value without prefix or "rounds=". The salt may be null,
	 *            in which case a salt is generated for you using
	 *            {@link ThreadLocalRandom}; for more secure salts consider using
	 ...
	 * A salt is generated for you using {@link ThreadLocalRandom}; for more secure
	 * salts consider using {@link SecureRandom} to generate your own salts and
	 * calling {@link #crypt(String, String)}.
	 * </p>
	 *
	 ...
	 * </ul>
	 * The magic strings {@code "$apr1$"} and {@code "$2a$"} are not recognized by
	 * this method as its output should be identical with that of the libc
	 * implementation.
	 * <p>
	 * The rest of the salt string is drawn from the set {@code [a-zA-Z0-9./]} and
	 * is cut at the maximum length of if a {@code "$"} sign is encountered. It is
	 ...
	 * </pre>
	 * <p>
	 * The resulting string starts with the marker string ({@code $n$}), where n is
	 * the same as the input salt. The salt is then appended, followed by a
	 * {@code "$"} sign. This is followed by the actual hash value. For DES the
	 * string only contains the salt and actual hash. The total length is dependent
	 ...
	 * </pre>
	 * <p>
	 * This method comes in a variation that accepts a byte[] array to support input
	 * strings that are not encoded in UTF-8 but e.g. in ISO-8859-1 where equal
	 * characters result in different byte values.
	 *
