 ...
 * specification. For extensibility, an array of special US-ASCII characters can
 * be specified in order to perform proper URI encoding for the different parts
 * of the URI.
 * <p>
 * This class is immutable. It is also thread-safe besides using BitSet which is
 * not thread-safe, but its public interface only call the access
 * </p>
 *
 * @see <a href=
 *      "https://tools.ietf.org/html/rfc3986#section-2.1">Percent-Encoding</a>
 * @since 1.12
 */
public class PercentCodec implements BinaryEncoder, BinaryDecoder {

	/**
	 * The escape character used by the Percent-Encoding in order to introduce an
	 * encoded character.
	 */
	...
	/**
	 * The minimum and maximum code of the bytes that is inserted in the bit set,
	 * used to prevent look-ups
	 */
	private int alwaysEncodeCharsMin = Integer.MAX_VALUE, alwaysEncodeCharsMax = Integer.MIN_VALUE;

	/**
	 * Constructs a Percent coded that will encode all the non US-ASCII characters
	 * using the Percent-Encoding while it will not encode all the US-ASCII
	 * characters, except for character '%' that is used as escape character for
	 * Percent-Encoding.
	 */
	public PercentCodec() {
	...
	/**
	 * Constructs a Percent codec by specifying the characters that belong to
	 * US-ASCII that should always be encoded. The rest US-ASCII characters will not
	 * be encoded, except for character '%' that is used as escape character for
	 * Percent-Encoding.
	 *
	 * @param alwaysEncodeChars
	 *            the unsafe characters that should always be encoded
	 * @param plusForSpace
	 *            the flag defining if the space character should be encoded as '+'
	 */
	...
	/**
	 * Inserts a single character into a BitSet and maintains the min and max of the
	 * characters of the {@code BitSet alwaysEncodeChars} in order to avoid look-ups
	...
	/**
	 * Percent-Encoding based on RFC 3986. The non US-ASCII characters are encoded,
	 * as well as the US-ASCII characters that are configured to be always encoded.
	 */
	@Override
	...
	/**
	 * Decode bytes encoded with Percent-Encoding based on RFC 3986. The reverse
	 * process is performed in order to decode the encoded characters to Unicode.
	 */
	@Override
	...
	/**
	 * Encodes an object into using the Percent-Encoding. Only byte[] objects are
	 * accepted.
	 *
	 * @param obj
