...
public class BaseNCodecInputStream extends FilterInputStream {
	...
	public int read(final byte b[], final int offset, final int len) throws IOException {
		if (b == null) {
			...
			int readLen = 0;
			/*
			 * Rationale for while-loop on (readLen == 0): ----- Base32.readResults()
			 * usually returns > 0 or EOF (-1). In the rare case where it returns 0, we just
			 * keep trying.
			 * 
			 * This is essentially an undocumented contract for InputStream implementors
			 * that want their code to work properly with java.io.InputStreamReader, since
			 * the latter hates it when InputStream.read(byte[]) returns a zero.
			 * Unfortunately our readResults() call must return 0 if a large amount of the
			 * data being decoded was non-base32, so this while-loop enables proper interop
			 * with InputStreamReader for that scenario. ----- This is a fix for CODEC-101
			 */
			while (readLen == 0) {
